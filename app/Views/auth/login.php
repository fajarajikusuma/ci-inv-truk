...
<head>
    ...
    <style>
        * {
        ...
        .dark .inp:-webkit-autofill,
        .dark .inp:-webkit-autofill:focus {
...
<body>
            ...
            <div style="position:relative;z-index:1;width:100%;max-width:640px;padding:48px 48px;margin:auto;color:#fff">
                ...
                <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:36px">
                    <div class="feat">
                        ...
                        <div>
                            <p style="font-size:13px;font-weight:700;color:#fff;margin:0">Integrated System</p>
                            <p style="font-size:12px;color:rgba(165,180,252,0.7);margin:3px 0 0">Terintegrasi dan Transparan</p>
                        </div>
                    </div>
                    <div class="feat">
    ...
    <style>
        ...
        .dark #el-topbar .theme-btn {
            background: rgba(255, 255, 255, 0.07);
            border-color: rgba(255, 255, 255, 0.1)
        }

        .dark #el-topbar .dark-text-logo {
        ...
        .dark #el-error p:first-child {
            color: #fca5a5
        }

        .dark #el-error p:last-child {
            color: #f87171
        }
    ...
    <script>
        ...
        document.getElementById('username').addEventListener('input', function() {
            if (this.value.trim()) showError('username', 'error-username', false);
        });
        document.getElementById('password').addEventListener('input', function() {
            if (this.value) showError('password', 'error-password', false);
        });
        ...
        form.addEventListener('submit', function(e) {
            ...
            let hasErr = false;
            if (!u) {
                showError('username', 'error-username', true);
                hasErr = true;
            }
            if (!p) {
                showError('password', 'error-password', true);
                hasErr = true;
            }
            if (hasErr) {
                e.preventDefault();
